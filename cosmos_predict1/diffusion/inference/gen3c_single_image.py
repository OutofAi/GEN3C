...
def process_dynamic(args, pipeline, device):
...
@torch.no_grad()
def run_full_dynamic_demo(pipeline,  
                  device,
                  vipe_path: str,
                  output_path: str,
                  trajectory: str = "left",
    ...
    process_dynamic(args, pipeline, device)

    # return the saved path
    return os.path.join(output_dir, output_filename + '.mp4')
...
if __name__ == "__main__":
    ...
    pipeline, moge_model, device = load_models(args.checkpoint_dir, args.guidance)
    log.info(f'legacy args are:{args}')
    process(args, pipeline, moge_model, device)
