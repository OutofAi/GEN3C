...
@torch.no_grad()
def ray_triangle_intersection_torch_blocked(
    ray_origins: torch.Tensor,      # (H, W, 3)
    ray_directions: torch.Tensor,   # (H, W, 3)
    vertices: torch.Tensor,         # (N, 3)
    faces: torch.Tensor,            # (M, 3)
    device: torch.device,
    ray_block_size: int = 65536,    # tune to your VRAM
    tri_block_size: int = 2048,     # tune to your VRAM
    eps: float = 1e-7,
) -> torch.Tensor:
    H, W = ray_origins.shape[:2]
    R = H * W
    ro = ray_origins.reshape(-1, 3).to(device, torch.float32).contiguous()
    rd = ray_directions.reshape(-1, 3).to(device, torch.float32).contiguous()
    verts = vertices.to(device, torch.float32).contiguous()
    f = faces.to(device, torch.long).contiguous()

    v0 = verts[f[:, 0]]
    v1 = verts[f[:, 1]]
    v2 = verts[f[:, 2]]

    t_min = torch.full((R,), float("inf"), device=device, dtype=torch.float32)

    T = f.shape[0]
    for t0 in range(0, T, tri_block_size):
        t1 = min(t0 + tri_block_size, T)
        v0c = v0[t0:t1]; v1c = v1[t0:t1]; v2c = v2[t0:t1]
        e1 = v1c - v0c
        e2 = v2c - v0c

        for r0 in range(0, R, ray_block_size):
            r1 = min(r0 + ray_block_size, R)
            ro_b = ro[r0:r1]                       # (Rb,3)
            rd_b = rd[r0:r1]                       # (Rb,3)

            # (Rb,Tc,3) intermediates exist only for this small block
            h = torch.cross(rd_b[:, None, :], e2[None, :, :], dim=2)
            a = (e1[None, :, :] * h).sum(dim=2)
            mask = a.abs() >= eps

            f_inv = torch.zeros_like(a)
            f_inv[mask] = 1.0 / a[mask]

            s = ro_b[:, None, :] - v0c[None, :, :]
            u = f_inv * (s * h).sum(dim=2)
            mask &= (u >= 0.0) & (u <= 1.0)

            q = torch.cross(s, e1[None, :, :], dim=2)
            v_bary = f_inv * (rd_b[:, None, :] * q).sum(dim=2)
            mask &= (v_bary >= 0.0) & ((u + v_bary) <= 1.0)

            t = f_inv * (e2[None, :, :] * q).sum(dim=2)
            mask &= t > eps

            t_valid = torch.where(mask, t, torch.full_like(t, float("inf")))
            t_min[r0:r1] = torch.minimum(t_min[r0:r1], t_valid.min(dim=1).values)

    depth = torch.where(torch.isinf(t_min), torch.zeros_like(t_min), t_min)
